...
class Solver:
    ...
    def solve(self, heuristic):
        ...
        if not root.state.is_solvable():    # Unsolvable.
            return None
        if heuristic != Solver.uniform_cost:    # Goal states only needed for heuristics.
        ...
        while self.frontier:
            ...
            if cur_node.state.is_goal_state():  # Solved!
                ...
                return solution
            cur_position = tuple(cur_node.state.position_flat)  # Tuples are hashable.
            if cur_position not in self.expanded:   # Avoid expanding nodes already expanded.
                ...
                for direction in cur_node.state.possible_moves():
                    ...
                    if tuple(new_node.state.position_flat) not in self.expanded:    # Only visit nodes not expanded yet.
                        self.num_visited += 1
...
class Puzzle:  
    ...
    def __init__(self, position):
        ...
        self.position = reshape(position, self.width)   # 2D representation of the puzzle.
        self.blank_ix = self.find(0)    # Indices of blank tile in 2D representation.
    ...
    def count_inversions(self):
        position = self.position_flat   # Use flat representation for convenience.
        count = 0
        for i in range(self.width**2):
            if position[i] != 0:    # Blank tile doesn't count in inversions.
                for j in range(i+1, self.width**2): # Traverse through the tiles which appear after this tile.
    ...
    def move_blank(self, direction):
        ...
        self.position_flat = flatten(self.position) # Update the flat representation of the puzzle.
    ...
    def is_goal_state(self):
        ...
        if state[0] != 0 and state[-1] != 0:    # Goal states always have blank tile at the first or last position.
            return False
...
def compare_arrays(array_1, array_2):
    if len(array_1) != len(array_2):    # Does not work with arrays of different lengths.
        return -1
...
def solve_n_times(width, heuristics, num_instances):
    ...
    while num_instances > 0:
        ...
        if start_state.is_solvable():   # Ignore unsolvable puzzles.
            ...
            sol_depths.append(len(solution.sol_path))   # Same depth for all heuristics due to optimality.
            num_instances -= 1
...
def plot_num_expanded_dict(sol_depths, num_expanded_dict):
    ...
    for heuristic, color in zip(num_expanded_dict.keys(), colors[:len(num_expanded_dict)]):
        plt.plot(sol_depths, num_expanded_dict[heuristic], '{}.'.format(color), label=heuristic)
    plt.legend(loc='best')
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(err_msg)
    if sys.argv[1] == '-r' and len(sys.argv) == 3:  # Mass solving of random puzzles.
        ...
        sol_depths, num_expanded_dict = solve_n_times(3, heuristics, num_instances)
        ...
        if sys.argv[1] == '-r': # Solve just 1 random puzzle.
            ...
            if len(sys.argv) < 3 or sys.argv[2] == 'manhattan': # Default heuristic is Manhattan distance.
